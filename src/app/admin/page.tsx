    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (data.url) {
          ...
          setMediaGallery((prev) => [data.url, ...prev]);
          setRecentUploadInfo({
            url: data.url,
                    ...
                    <div className="space-y-3 pt-3 border-t border-white/10">
                      <div className="flex items-center justify-between">
                        <label className="text-xs font-bold text-cyan-300 flex items-center gap-1.5 uppercase font-mono">
                          <Cloud className="w-4 h-4" /> Cloudinary Product Photos *
                        </label>
                      ...
                      <div className="border-2 border-dashed border-cyan-500/30 hover:border-cyan-400 rounded-2xl p-6 text-center bg-cyan-950/10 relative transition-colors">
                        <input
                          type="file"
                          accept="image/*"
                          multiple
                        ...
                        <p className="text-xs font-bold text-white">Click or drag & drop high-res product photos</p>
                        <p className="text-[10px] text-gray-400 mt-0.5">Supports PNG, JPG, WEBP • Uploads directly to Cloudinary</p>
                        {isUploading && (
                          <div className="mt-2 text-xs font-bold text-cyan-400 animate-pulse flex items-center justify-center gap-1.5">
                            <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Uploading to Cloudinary CDN...
                          </div>
                        )}
                      </div>
                      ...
                      {uploadedUrls.length > 0 && (
                        <div className="space-y-2">
                          <span className="text-[11px] text-gray-300 font-mono">Uploaded Images ({uploadedUrls.length}):</span>
                          <div className="grid grid-cols-4 gap-2">
                            {uploadedUrls.map((url, i) => (
                              <div key={i} className="relative h-20 rounded-xl overflow-hidden border border-white/20 group">
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="p-6 sm:p-8 rounded-3xl bg-[#0d0f1a] border border-white/10 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="border-2 border-dashed border-amber-500/30 hover:border-amber-400 rounded-2xl p-6 text-center bg-amber-950/10 relative transition-colors">
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleFileUpload}
                  ...
                  <p className="text-xs font-bold text-white">Click or drag & drop Hero Banner graphic</p>
                  <p className="text-[10px] text-gray-400">Supports high-res PNG, JPG, WEBP • Saves to Cloudinary CDN</p>
                </div>
        ...
        {activeTab === 'media' && (
          <div className="space-y-6">
            <div className="p-6 rounded-3xl bg-[#0d0f1a] border border-white/10 space-y-6">
              ...
              {/* Upload Dropzone */}
              <div className="border-2 border-dashed border-sky-500/30 hover:border-sky-400 rounded-2xl p-8 text-center bg-sky-950/10 relative transition-colors">
                <input
                  type="file"
                  accept="image/*"
                  multiple
                ...
                <p className="text-sm font-bold text-white">Upload Media Assets directly to Cloudinary</p>
                <p className="text-xs text-gray-400 mt-1">Multi-file drag & drop enabled</p>
              </div>
              ...
              <div className="flex items-start justify-between gap-4 pb-4 border-b border-white/10">
                <div className="space-y-1">
                  <h2 className="text-xl font-bold text-white font-mono flex items-center gap-2">
                    ...
                    Supabase SQL Database Console
                  </h2>
                  <p className="text-xs text-gray-400">
                    Copy and run this SQL script inside your Supabase project's SQL Editor to instantiate all tables and RLS policies.
                  </p>
                </div>
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-pink-500 hover:bg-pink-400 text-black font-extrabold text-xs uppercase tracking-wider transition-all flex items-center gap-2 shadow-lg shadow-pink-500/20 shrink-0"
                >
